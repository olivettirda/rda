...
-- ============================================
-- 클라이언트 사이드 암호화 (E2EE) 안내
-- ============================================
-- 노트 content 필드는 클라이언트에서 AES-256-GCM으로 암호화됩니다.
-- 암호화된 데이터 형식: ENC:v1:<base64_iv>:<base64_ciphertext>
-- 암호화 키는 사용자 비밀번호에서 PBKDF2로 파생됩니다.
--
-- 중요 사항:
-- 1. 기존 미암호화 노트는 사용자가 로그인할 때 자동으로 암호화됩니다.
-- 2. DB 관리자도 암호화된 노트 내용을 읽을 수 없습니다.
-- 3. 사용자가 비밀번호를 변경하면 암호화 키가 변경되므로,
--    비밀번호 변경 시 기존 노트를 새 키로 재암호화해야 합니다.
-- 4. 노트 전달 시 수신자가 처음 로드할 때 자동으로 암호화됩니다.
-- 5. content 필드의 TEXT 타입은 변경 불필요 (암호화된 데이터도 TEXT로 저장)
